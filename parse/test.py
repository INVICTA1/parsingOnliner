...
def parse_other_pages(browser, amount_page, url, data_time):
...
def save_result_csv(flats):
    ...
    with open(path, 'w', encoding='cp1251',  newline='') as file:
        ...
        writer.writerow(['Prise USD: ', 'Prise BYN: ', 'Time: ', 'Address: ', 'room: ', 'link: '])
        for flat in flats:
...
def parseOnliner():
    ...
    flats = parse_other_pages(browser, amount_page, URL, data_time)
    browser.quit()
